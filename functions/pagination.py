...
def createButtons(disabledComponents, client): 
    return [
        [
...
async def pagination(client, ctx, message, embeds):
    ...
    mainMessage = await ctx.send(embed=embeds[page], components=createButtons(False, client))
    while going:
        ...
        await interaction.respond(type=7, embed=embeds[page], components=createButtons(disabledComponents, client))